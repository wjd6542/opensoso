...
$(".insert, .update").on("click",function(){
	...
	{
		if($.util.nullCheckForm("code","코드값은 필수 입니다.","form1")) return false;
		if($.util.nullCheckForm("title","코드명은 필수 입니다.","form1")) return false;
	...
	{
		if($.util.nullCheckForm("code","코드값은 필수 입니다.","form2")) return false;
		if($.util.nullCheckForm("title","코드명은 필수 입니다.","form2")) return false;
...
$(".check").on("click",function(){
	var url = "/overlapCheck.do";
	var value = "";
	var result = {};
	// 환경설정 코드 아이디 배열
	var idArray = new Array("code","actionType");
	var codeArray = new Array("systemCode","code","actionType");
	var actionType = $(this).attr("contextmenu");
	...
	else
	{
		if($.util.nullCheckForm("code","코드값은 필수 입니다.", "form2")) return false;
		$("#form2 #actionType").val(actionType);
		result = $.util.jsonObjectMk(codeArray,"form2");
	}
	// 아작스 처리
